...
/**
 * Some tweaks:
 *   define( 'MEMCACHE_SITE_PREFIX', 'EX' );
 *       Prefix all of the cache keys with 'EX', in case you need to
 *       share a memcache with another wordpress install.
 *   define( 'MEMCACHE_DEFAULT_SERVERS', 'cache1:11211,cache2:11211' );
 *       Servers to use for the 'default' bucket
 *
 * This version of the library reuses the cache object and memcached
 * connection object instead of creating new ones when wp_cache_init()
 * is called. This seems to speed things up and resolve some strange
 * problems while using memcache.
 *
 **/

global $memcached_servers;

# Check to see if we've already defined servers in either the
# $memcached_servers global or in the constant
if ( empty( $memcached_servers ) && defined( 'MEMCACHE_DEFAULT_SERVERS' ) )
    $memcached_servers = explode( ',', MEMCACHE_DEFAULT_SERVERS );
...
function wp_cache_init() {
    if ( !empty($GLOBALS['wp_object_cache']) )
        $GLOBALS['wp_object_cache']->setup_prefixes();
    else
...
class WP_Object_Cache {
	...
	function key($key, $group) {	
		...
		return preg_replace('/\s+/', '', $this->site_prefix . "$prefix$group:$key");
	}
    ...
    function setup_prefixes() {
        ...
        $this->site_prefix = defined( 'MEMCACHE_SITE_PREFIX' ) ? MEMCACHE_SITE_PREFIX . ':' : '';
    }
}
